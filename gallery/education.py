...
"P33": "Each rod aligns its axis — not its direction — with neighbours, so the order is apolar: there is no single flock heading. Self-propulsion along the rod axis continually creates and annihilates ±1/2 topological defects, the half-integer singularities unique to nematic (head-tail-symmetric) order. The result is 'active turbulence': sustained local alignment threaded by motile defects, with no global direction and no controller.",
"P34": "A tie between disagreeing nodes is either rewired (drop it, link instead to someone who already agrees) or resolved by one node adopting the other's view. When rewiring dominates, agreement and connection reinforce each other — like-minded nodes cluster and cross-cutting ties vanish — until the network splits into disconnected same-opinion communities. The structure itself, not just the states, self-organises: the topology is an emergent outcome of the coupled dynamics.",
"P35": "Hard (purely repulsive) disks have no attraction, yet above a packing fraction they spontaneously arrange into a six-fold (hexatic) lattice. The driver is entropy: an ordered packing leaves each particle more local wiggle room than a jammed disordered one, so the ordered state has higher entropy and is favoured. Order emerges from excluded volume alone — a counterintuitive, purely geometric route to structure.",
"P36": "Agents repeatedly trade a conserved resource in random pairs, each keeping a fixed personal fraction (its saving propensity) before splitting the rest. When everyone saves at the same rate the result is a mild, Gamma-shaped spread; but when saving rates DIFFER across agents, the steady state develops a true power-law (Pareto) tail — a few agents accumulate a heavy-tailed share. The inequality is not driven by luck or skill but by heterogeneity itself: the distribution of behaviours, not the trades, generates the macroscopic power law.",
"P37": "Several species draw down the same handful of shared resources, each limited by whichever resource is scarcest for it. Classical theory says no more species can persist than there are limiting resources (competitive exclusion). But when the competition is structured cyclically — each species best at one resource, heaviest consumer of another — the system never reaches equilibrium: abundances oscillate and turn chaotic, and that perpetual motion lets many species coexist on a few resources. Non-equilibrium dynamics, not a static niche split, sustains the diversity.",
}
...
"P33": "Active nematics in microtubule-kinesin suspensions, dense bacterial colonies, and confluent cell monolayers, where ±1/2 defects drive flows and even cell extrusion (biology); active liquid crystals and self-propelled colloids (physics/materials).",
"P34": "Opinion and social networks that polarise as people rewire who they listen to (social science); coevolving contact networks in epidemics and adaptive infrastructure/communication networks (technology).",
"P35": "Colloidal and granular crystallization, hard-sphere freezing, and depletion-driven self-assembly (physics/materials); the classic Alder-Wainwright result that launched molecular-dynamics simulation (computational science).",
