...
class TestGreenbergHastingsCA:
    def test_setup_and_state_keys(self):
        ...
        m = GreenbergHastingsCA(grid_size=20, seed=0)
        state = m.setup()
        assert "grid" in state
        assert "step" in state
        assert "n_excited" in state
        assert "n_refractory" in state
        assert "n_rest" in state
        assert "grid_dims" in state
        assert state["grid_dims"] == (20, 20)

    def test_state_counts_sum_to_grid_size(self):
        from epc.models.greenberg_hastings import GreenbergHastingsCA
        m = GreenbergHastingsCA(grid_size=30, seed=42)
        h = m.run(50)
        for state in h:
            total = state["n_excited"] + state["n_refractory"] + state["n_rest"]
    ...
    def test_high_threshold_quiescence(self):
        ...
        from epc.models.greenberg_hastings import GreenbergHastingsCA
        m = GreenbergHastingsCA(grid_size=20, threshold=9, density=0.05, seed=0)
        h = m.run(30)
        final = h[-1]
        assert final["n_excited"] == 0
    ...
    def test_broken_wave_sustains_activity(self):
        ...
        h = m.run(200)
        late_excited = [s["n_excited"] for s in h[-50:]]
        assert max(late_excited) > 0, "Spiral activity died"

    def test_convergence_detection(self):
        from epc.models.greenberg_hastings import GreenbergHastingsCA
        # threshold=9 means no propagation; need enough steps for all
        # refractory cells to decay (n_refractory+1 steps after last excitation)
        m = GreenbergHastingsCA(grid_size=10, threshold=9, density=0.05, n_refractory=3, seed=0)
        m.run(50)
        assert m.is_converged()

    def test_timescale(self):
        from epc.models.greenberg_hastings import GreenbergHastingsCA
        m = GreenbergHastingsCA(grid_size=50)
        assert m.get_timescale() == 50.0

    def test_metadata(self):
        from epc.models.greenberg_hastings import GreenbergHastingsCA
        m = GreenbergHastingsCA(grid_size=40, n_refractory=5, threshold=2)
        md = m.get_metadata()
        assert md["model_name"] == "greenberg_hastings_ca"
        assert md["model_class"] == "excitable_medium"
        assert md["grid_size"] == 40
        assert md["n_refractory"] == 5
...
class TestVicsekModel:
    def test_setup_and_state_keys(self):
        ...
        m = VicsekModel(n_agents=50, seed=0)
        state = m.setup()
    ...
    def test_constant_speed(self):
        ...
        from epc.models.vicsek import VicsekModel
        m = VicsekModel(n_agents=50, speed=0.5, seed=0)
        h = m.run(100)
        for state in h:
    ...
    def test_metadata(self):
        from epc.models.vicsek import VicsekModel
        m = VicsekModel(n_agents=200, noise=0.3, attraction_strength=0.1)
        md = m.get_metadata()
        assert md["model_name"] == "vicsek_flocking"
        assert md["model_class"] == "active_matter"
        assert md["n_agents"] == 200
        assert md["noise"] == 0.3
        assert md["attraction_strength"] == 0.1
