...
Sprint 46 completes the continuous_2d dim4 batch, running the Phase-2a v1.2 panel against all three continuous_2d patterns now that Sprint 37 generators and Class B supplements are in place.
...
Sprint 47 closes the lattice_1d (P8) and oscillator (P10) dim4 depth-gaps by running the Phase-2a v1.2 panel. The sprint also introduces the "sequence" substrate format for lattice_1d detectors, enabling the generic Class A and Class B substrate generators to feed P8.

**P8 Nagel-Schreckenberg / traffic jamming — Phase-2a panel v1.2 (Sprint 47).**
...
The P21 polarization detector identifies persistent multimodality in agent opinion distributions via Hartigan's dip test (Hartigan & Hartigan 1985) on the final opinion distribution. The canonical positive is the Hegselmann-Krause (2002) bounded-confidence model at ε = 0.2 (canonical fragmented regime), producing two stable opinion clusters at approximately 0.30 and 0.70 from uniform initial conditions. Sprint 48 added the `opinions` detector_format to the Phase-2a panel harness: Class A synthetic substrates produce continuous unimodal opinion distributions; catalog mates and network supplements are adapted via a rank transform to uniform [0, 1] (unimodal); Class C runs HK at high ε ∈ linspace(0.45, 0.60, 10) (consensus regimes where the model converges to a single cluster).
...
Sprint 49 is a panel-metadata sprint: no detector logic, model, or primary-metric changes. The deliverable is a batch update of the `DETECTOR_INVARIANCE_FLAGS` dict in `epc/phase2a/detector_invariance.py`, re-running the six affected panels under v1.2 to confirm expected improvements.

**P1 Schelling / segregation (Sprint 49 re-run).**
Flag added: `time_shuffle_invariant=True`. Justification: P1's Moran's I is computed independently per lattice frame; a Schelling trajectory that has converged to segregation has every frame showing high I, regardless of temporal order. The `time_shuffled` substrate is therefore degenerate-by-construction. Result: `time_shuffled` SKIPPED; syn TNR advances from 0.800 to 0.889. Residual FPs: `linear_gradient` at CONFIRMATION (C-p1-linear-gradient-fp; Moran's I correctly responds to spatial gradient structure) and Class C calibration issues (fai TNR = 0.400; C-p1-class-c-subthreshold-fp). Overall TNR 0.704 → 0.731, Cohen's d 1.624 → 1.740. Verdict: **PARTIAL** (unchanged; Class C calibration fix out-of-scope Sprint 49).

**P2 ABP / MIPS (Sprint 49 re-run).**
Flag added: `permutation_invariant=True`. Justification: `two_phase_score` computes gas/liquid phase fractions by thresholding a coarse density grid; the density grid is defined by particle positions, not particle identities, so shuffling particle indices does not change the spatial density field. Result: `permutation_shuffled` SKIPPED; syn TNR advances from 0.900 to 1.000. C-p2-perm-shuffled-fp CLOSED. Overall TNR 0.958 → 1.000, Cohen's d 3.401 → 4.245. Verdict: **PASS** (clean; `permutation_shuffled` was the only FP).

**P5 Vicsek / flocking (Sprint 49 re-run).**
Flag updated: `time_shuffle_invariant=False → True`. Justification: P5's mean polar order parameter φ = |⟨e^{iθ}⟩| is computed per frame; in a flocked trajectory every frame has high φ, so temporal reordering does not change the metric distribution. Result: `time_shuffled` SKIPPED (in addition to already-skipped `permutation_shuffled`); syn TNR advances from 0.889 to 1.000. C-p5-time-shuffle-fp CLOSED. Overall TNR 0.957 → 1.000, Cohen's d 4.987 → +inf. Verdict: **PASS** (clean; strengthened from PASS-with-weakness).

**P6 D'Orsogna / milling (Sprint 49 re-run).**
Flags added: `permutation_invariant=True`, `time_shuffle_invariant=True`. Justification: |L| = |Σ r_i × v_i|/N is a sum over particles (permutation-invariant) and in a milling trajectory every frame has the swarm in its milled configuration with high |L|, so temporal reordering is irrelevant. Result: both `permutation_shuffled` and `time_shuffled` SKIPPED; syn TNR advances from 0.900 to 1.000. C-p6-time-shuffle-fp CLOSED. Overall TNR 0.958 → 1.000, Cohen's d 5.087 → +inf. Verdict: **PASS** (clean; strengthened from PASS-with-weakness).

**P8 Nagel-Schreckenberg / traffic jamming (Sprint 49 re-run).**
Flags added: `permutation_invariant=True`, `time_shuffle_invariant=True`. Justification: P8's `stopped_fraction` is a cell-occupancy average; the NaSch model is translation-invariant (cells are anonymous), so permuting cell indices is immaterial, and the time-average is preserved under any temporal reordering. Result: both Class A FPs SKIPPED; syn TNR advances from 0.800 to 1.000. C-p8-perm-shuffled-fp and C-p8-time-shuffle-fp CLOSED. Class C calibration issue (fai TNR = 0.400; 6/10 regimes at or above NS jamming onset at rho ≈ 0.12) unchanged — carry-forward C-p8-class-c-near-onset remains open; a separate sprint will restrict the Class C sweep to rho ∈ linspace(0.01, 0.09). Overall TNR 0.652 → 0.714, Cohen's d 1.751 → 1.772. Verdict: **PARTIAL** (unchanged; Class C calibration fix out-of-scope).
