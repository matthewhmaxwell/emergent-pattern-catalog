...
| P9 | Temporal synchronization (Kuramoto) | PASS | PASS | PASS | PARTIAL | GAP | dim4: K_c=2γ analytical confirmation but no documented Phase-2a sweep against substrate-diverse non-positives. Phase-2a panel v1.0 (Sprint 30): overall TNR=0.733, failed_regime_tnr=1.000, catalog_tnr=0.400, see `analysis/outputs/p9_phase2a_panel.json`. Class C (sub-K_c) clean; Class A/B losses are panel-adapter artifacts (binary grids → bimodal phase distributions). Carry-forward C-panel-spec. | M |
...
| P14 | Self-organized criticality (BTW) | PASS | PARTIAL | PASS | PARTIAL | GAP | dim2: τ=1.247 from a single 100k-event run; ≥5-seed bootstrap dispersion not reported; dim4: dissipative null is a strong content-level test but not a substrate-diverse Phase-2a sweep | M |
| P15 | Persistent propagating computation (GoL) | PASS | N/A | PASS | PASS | AT-DEPTH | dim2 N/A: GoL is fully deterministic and the canonical positive (R-pentomino, glider collisions) is not phase-boundary-dependent; dim4: TE discriminator + multi-checkpoint reproducibility + GH/SIR/Nowak-May rejection collectively constitute a substrate-diverse Phase-2a sweep | — |
| P18 | Collective consensus / decision-making (voter) | PASS | PASS | PASS | PASS | AT-DEPTH | Sprint 20/21/24 produced canonical voter L=64 positive, multi-seed validation across L ∈ {64,128,256}, substantive ADR 54/55/56 methods notes, six-row discriminator rejection table including Schelling false-positive closure (Sprint 24 #20b). Phase-2a panel v1.0 (Sprint 30): overall TNR=0.767, syn_tnr=1.000, cat_tnr=0.900, fai_tnr=0.400, see `analysis/outputs/p18_phase2a_panel.json`. Class C (high-bias init) returned PARTIAL — voter has no parameter regime that suppresses consensus, so Class C used init_fraction proxies; carry-forward C-panel-spec for Sprint 31 panel-spec revision. AT-DEPTH grade unchanged (existing audit-cited discriminators stand). | — |
