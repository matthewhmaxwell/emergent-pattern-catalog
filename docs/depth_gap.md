...
| P9 | Temporal synchronization (Kuramoto) | PASS | PASS | PASS | PARTIAL | GAP | dim4: K_c=2γ analytical confirmation but no documented Phase-2a sweep against substrate-diverse non-positives. Phase-2a panel v1.1 still PARTIAL (Sprint 32): overall TNR=0.913 (<0.95), syn=0.800, cat=1.000 (advisory n=3), fai=1.000, Cohen's d=3.445, see `analysis/outputs/p9_phase2a_panel.json`. v1.1 catalog substrate-typed fix worked (cat: 0.400→1.000, fai stayed at 1.000); residual 2 false positives are degenerate-sync Class A substrates (constant-phase field; permutation-shuffled positive — both preserve r). Carry-forward C-class-a-oscillator-degenerate flagged for chat-led panel-spec revision (likely v1.2). Detector NOT modified per Sprint 30 rule. | M |
...
| P14 | Self-organized criticality (BTW) | PASS | PARTIAL | PASS | PARTIAL | GAP | dim2: τ=1.247 from a single 100k-event run; ≥5-seed bootstrap dispersion not reported; dim4: dissipative null is a strong content-level test but not a substrate-diverse Phase-2a sweep. Phase-2a panel v1.1 still PARTIAL (Sprint 33): overall TNR=0.889 (<0.95), syn=0.800, cat=1.000 (7/7 lattice_2d mates), fai=0.900 (1 borderline at p_diss=0.350), Cohen's d=4.779, see `analysis/outputs/p14_phase2a_panel.json`. Class A losses match the C-class-a-degenerate pattern from Sprint 32 (oscillator) — v1.2 generalization needed. Class C borderline is a P14-specific edge effect (C-p14-class-c-borderline, low priority). Detector NOT modified per Sprint 30 rule. | M |
