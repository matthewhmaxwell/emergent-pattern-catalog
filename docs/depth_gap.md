...
|---|---|---|---|---|---|---|---|---|
| P1 | Similarity-driven aggregation (Schelling) | PASS | PASS | PASS | PARTIAL | GAP | dim4: Phase-2a panel v1.2 PARTIAL (Sprint 42): overall TNR=0.593, syn=0.800 (FPs: `time_shuffled` — Schelling intermediate frames already segregated; `linear_gradient` — Moran's I responds to gradient structure), cat=0.571 (FPs: P11_LV, P15_GoL, P12_RPS — spatial autocorrelation in other lattice_2d models), fai=0.400 (FPs: threshold=0.050 and threshold∈[0.161,0.250] — accidental initial clustering), Cohen's d=1.298. Sprint 30 rule: no detector change; carry-forwards C-p1-time-shuffle-fp, C-p1-linear-gradient-fp, C-p1-class-b-lattice2d-fp, C-p1-class-c-subthreshold-fp opened. See `analysis/outputs/p1_phase2a_panel.json`. | M |
...
- **At-depth: 6 / 19** (P9, P15, P18, P27, P28, P31). P27 added in Sprint 40 via Phase-2a panel v1.2 PASS (TNR=1.000, Cohen's d=2.950). P9 added in Sprint 35 via Phase-2a panel v1.2 PASS-with-weakness. P22 dim4 advanced to PASS in Sprint 41 (TNR=1.000, Cohen's d=+inf) via irreversibility prereq, but dims 1–3 remain PARTIAL so P22 does not yet reach AT-DEPTH.
- **Gap: 13 / 19**. P22 dim4 now PASS (Sprint 41), dims 1–3 still PARTIAL; remaining dim4 gaps unchanged.
- **Most common FAIL/PARTIAL dimension: dim4 (broad negative sweep)** — 14/19 patterns scored PARTIAL on dim4. Cross-detection transfer matrix coverage is universal but the rubric explicitly calls this insufficient (a Phase-2a-style sweep specifically tests detector specificity against models *not* in the catalog). Only P15, P18, P27, P28, P31 have content-level discriminators or mechanistic-null gates that meet the dim4 PASS bar.
