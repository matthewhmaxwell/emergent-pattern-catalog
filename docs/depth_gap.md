...
| P22 | Information cascade / social contagion (SIR) | PARTIAL | PARTIAL | PARTIAL | PARTIAL | GAP | dim1: Datta & Acharyya CA cited (Sprint 8 reference correction); percolation transition replicated qualitatively but no specific Fig/table quantitative match documented; dim2: per-N basin fraction not reported; dim3: methods note exists for substitution from Fuks-Lawniczak but light on integration choices; dim4: Phase-2a panel v1.2 PARTIAL (Sprint 39): overall TNR=0.519, syn=0.900, cat=0.714, fai=0.000 — Class C failed-regime params lie above Moore p_c≈0.038. Sprint 40 re-run after correcting Class C to infection_prob ∈ [0.005, 0.030]: overall TNR=0.889, syn=0.900, cat=0.714, fai=1.000, Cohen's d=2.981, PARTIAL — Class C fixed but catalog false positives (LV, RPS) remain; C-p22-class-c-above-percolation CLOSED; see `analysis/outputs/p22_phase2a_panel.json` | L |
...
- **At-depth: 6 / 19** (P9, P15, P18, P27, P28, P31). P27 added in Sprint 40 via Phase-2a panel v1.2 PASS (TNR=1.000, Cohen's d=2.950). P9 added in Sprint 35 via Phase-2a panel v1.2 PASS-with-weakness. Sprint 39 panel runs for P22 (PARTIAL) and P27 (FAIL) did not add new AT-DEPTH patterns; Sprint 40 resolved P27.
- **Gap: 13 / 19**. P22 remains GAP on dim4 (PARTIAL, TNR=0.889 after Sprint 40 Class C fix; catalog false positives require discrimination sprint).
