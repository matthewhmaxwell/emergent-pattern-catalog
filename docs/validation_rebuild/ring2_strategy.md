...
| `persistent_homology` | topology / loops + voids | **ADMITTED** (this sprint; field path added) | TWO substrate paths. Positions (Rips): `h1_max` (single dominant loop) ring 0.58/vortex 0.43 vs non-loop ≤0.12 (gap +0.31); NOT `h1_total` (noise-confounded). FIELD (superlevel-set holes, scipy.ndimage): `field_loop_area` (persistent enclosed-hole area, gaussian-denoised) — RD-worms 0.23/stripes 0.38/ring 0.21/swiss 0.13 vs blob 0/spots 0/noise 0.009 (gap +0.12); NOT raw hole count (noise-confounded — same lesson). Closes the field-topology gap (RD/Lenia). |
...
| `vorticity` | rotational order / circulation (morphology gap) | **ADMITTED** (ceiling-raise round) | net angular momentum about the COM, `ang_mom = |Σ r×v| / Σ|r||v|`. Rotation (synthetic 1.00, vortex_milling 0.986) vs all non-rotational ≤0.23 (flocking/translation/nematic/random/static), gap +0.76. New axis distinct from polar/nematic alignment (a milling ring has low polar order) and clustering. Agent substrates (velocities, or derived from positions). |
